...
select 'DimSecurity row count', BatchID, Result, 'Actual row count matches or exceeds Audit table minimum' from (
	select distinct BatchID, case when
		cast((select MessageData from DImessages where MessageType = 'Validation' and BatchID = a.BatchID and MessageSource = 'DimSecurity' and MessageText = 'Row count') as UNSIGNED) >= 
...
select 'DimCompany row count', BatchID, Result, 'Actual row count matches or exceeds Audit table minimum' from (
	select distinct BatchID, case when
		cast((select MessageData from DImessages where MessageType='Validation' and BatchID = a.BatchID and MessageSource = 'DimCompany' and MessageText = 'Row count') as UNSIGNED) <= 
...
select 'FactWatches row count', BatchID, Result, 'Actual row count matches Audit table' from (
	select distinct BatchID, (
		case when 
			cast((select MessageData from DImessages where MessageSource = 'FactWatches' and MessageType = 'Validation' and MessageText = 'Row count' and BatchID = a.BatchID) as unsigned) -
			cast((select MessageData from DImessages where MessageSource = 'FactWatches' and MessageType = 'Validation' and MessageText = 'Row count' and BatchID = a.BatchID-1) as unsigned) =
...
select 'FactWatches active watches', BatchID, Result, 'Actual total matches Audit table' from (
	select distinct BatchID, case when
		(select cast(MessageData as unsigned) from DImessages where MessageSource = 'FactWatches' and MessageType = 'Validation' and MessageText = 'Row count' and BatchID = a.BatchID) +
		(select cast(MessageData as unsigned) from DImessages where MessageSource = 'FactWatches' and MessageType = 'Validation' and MessageText = 'Inactive watches' and BatchID = a.BatchID) =
...
select 'FactMarketHistory row count', BatchID, Result, 'Actual row count matches Audit table' from (
	select distinct BatchID, (
		case when 
			cast((select MessageData from DImessages where MessageSource = 'FactMarketHistory' and MessageType = 'Validation' and MessageText = 'Row count' and BatchID = a.BatchID) as unsigned) -
			cast((select MessageData from DImessages where MessageSource = 'FactMarketHistory' and MessageType = 'Validation' and MessageText = 'Row count' and BatchID = a.BatchID-1) as unsigned) =
...
select 'FactHoldings row count', BatchID, Result, 'Actual row count matches Audit table' from (
	select distinct BatchID, (
		case when 
			cast((select MessageData from DImessages where MessageSource = 'FactHoldings' and MessageType = 'Validation' and MessageText = 'Row count' and BatchID = a.BatchID) as unsigned) -
			cast((select MessageData from DImessages where MessageSource = 'FactHoldings' and MessageType = 'Validation' and MessageText = 'Row count' and BatchID = a.BatchID-1) as unsigned) =
...
FROM (
    SELECT DISTINCT a.BatchID, m.MessageSource, 
        CAST(m1.MessageData AS UNSIGNED) AS RowsThisBatch, 
        CAST(m2.MessageData AS UNSIGNED) AS RowsLastBatch
    FROM Audit a 
    LEFT JOIN DImessages m ON m.BatchID = 0 AND m.MessageText = 'Row count' AND m.MessageType = 'Validation'
    LEFT JOIN DImessages m1 ON m1.BatchID = a.BatchID AND m1.MessageSource = m.MessageSource AND m1.MessageText = 'Row count' AND m1.MessageType = 'Validation'
    LEFT JOIN DImessages m2 ON m2.BatchID = a.BatchID-1 AND m2.MessageSource = m.MessageSource AND m2.MessageText = 'Row count' AND m2.MessageType = 'Validation'
    ...
    SELECT DISTINCT a.BatchID, m.MessageSource, 
        ...
        CAST(m2.MessageData AS UNSIGNED) AS RowsLastBatch
    FROM Audit a 
    RIGHT JOIN DImessages m ON m.BatchID = 0 AND m.MessageText = 'Row count' AND m.MessageType = 'Validation'
    RIGHT JOIN DImessages m1 ON m1.BatchID = a.BatchID AND m1.MessageSource = m.MessageSource AND m1.MessageText = 'Row count' AND m1.MessageType = 'Validation'
    RIGHT JOIN DImessages m2 ON m2.BatchID = a.BatchID-1 AND m2.MessageSource = m.MessageSource AND m2.MessageText = 'Row count' AND m2.MessageType = 'Validation'
    WHERE a.BatchID IN (1, 2, 3)
) o
...
FROM (
    SELECT DISTINCT a.BatchID, m.MessageSource, 
        CAST(m1.MessageData AS UNSIGNED) AS RowsUnjoined, 
        CAST(m2.MessageData AS UNSIGNED) AS RowsJoined
    FROM Audit a 
    LEFT JOIN DImessages m ON m.BatchID = 0 AND m.MessageText = 'Row count' AND m.MessageType = 'Validation'
...
from (
	select m1.MessageSource, sum( case when cast(m1.MessageData as unsigned) > cast(m2.MessageData as unsigned) then 1 else 0 end ) as regressions
	from DImessages m1 
